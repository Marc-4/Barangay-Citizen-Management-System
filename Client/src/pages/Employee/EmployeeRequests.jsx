...
const AdminRequests = () => {
  ...
  const getRequests = async () => {
    ...
    try {
      const body = null
      const method = 'GET'
      const route = `${import.meta.env.VITE_API_URL}/api/employee/requests?entries=20&filter=PENDING`
  ...
  const getPastRequests = async () => {
    ...
    try {
      const body = null
      const method = 'GET'
      const route = `${import.meta.env.VITE_API_URL}/api/employee/requests?entries=20&filter=HISTORY`
      console.log('calling API..')
      const data = await callAPI(body, method, route)
      if (data && data.result === 'OK') {
        setError(null)
  ...
  const handleSearch = async (query) => {
    ...
    try {
      const body = null
      const method = 'GET'
      const route = `${import.meta.env.VITE_API_URL}/api/employee/requests/search?query=${query}&filter=${filter}`
      const data = await callAPI(body, method, route)
      if (data.result === 'OK') {
